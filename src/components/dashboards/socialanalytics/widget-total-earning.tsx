...
const TotalEarningCard = ({ earningData, title, earning, trend, percentage, comparisonText, className }: Props) => {
  return (
    <Card className={cn('flex flex-col rounded-xl border py-6 shadow-sm justify-between gap-6 sm:min-w-0', className)}>
      <CardHeader className='px-6 py-0 flex items-center flex-row justify-between'>
        <span className='text-lg font-semibold'>{title}</span>
        <DropdownMenu modal={false}>
          <DropdownMenuTrigger asChild>
            <Button variant='ghost' className='text-muted-foreground size-6 rounded-full p-0'>
      ...
      <CardContent className='px-6 flex flex-1 flex-col gap-4 py-0'>
